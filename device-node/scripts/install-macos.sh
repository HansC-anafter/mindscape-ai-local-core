...
LAUNCHD_LABEL="ai.mindscape.device-node"
...
if command -v gemini &>/dev/null; then
    ...
    npm install -g @google/gemini-cli 2>/dev/null \
        ...
        || echo "   gemini-cli install failed (non-fatal, install manually: npm install -g @google/gemini-cli)"
fi

# 3. Create logs directory
mkdir -p "$PROJECT_DIR/logs"

# 4. Detect node binary
NODE_BIN="$(which node)"
if [ -z "$NODE_BIN" ]; then
...
# 6. Bootstrap and start the agent
...
echo ""
echo "  Status:  launchctl print $LAUNCHD_DOMAIN/$LAUNCHD_LABEL"
echo "  Logs:    tail -f $PROJECT_DIR/logs/device-node.log"
echo "  Stop:    launchctl bootout $LAUNCHD_DOMAIN/$LAUNCHD_LABEL"
echo "  Restart: launchctl kickstart -k $LAUNCHD_DOMAIN/$LAUNCHD_LABEL"
echo ""
echo "  After code changes: npm run install:macos"
