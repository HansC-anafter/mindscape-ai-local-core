...
> 外部執行環境（Gemini CLI、OpenClaw、LangGraph 等）提供執行環境。在 Mindscape 架構中，*agent 身份*由 Intent + Mind-Lens + Memory 定義；runtime 提供算力和沙箱來執行。
...
* **Agent（Mindscape 定義）** — Agent 是一個 **AgentSpec** 在 **Agent OS** 上的運行實例：
  - **AgentSpec** = **Agent Core**（Intent + Mind-Lens + Memory）+ **Actuator**（工具/能力包/允許副作用）
