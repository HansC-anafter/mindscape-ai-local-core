...
- 當成一個帶有可選 `project / flow / playbook` 執行層的 **端到端本地工作空間**
...
- **Meeting Runtime**：負責當下的 deliberation、釐清、收斂、派發與閉環
- **Governed Memory Fabric**：負責長期連續性、證據、升格、失效與 serving

而這個認知內核始終受到治理上下文約束：

- **Intent**：為什麼要做、什麼才重要
- **Mind-Lens**：用什麼視角看、如何取捨
- **Policy**：哪些底線不能破
...
2. **編譯治理上下文**
   ...
   - **Mind-Model VC** 將你願意提供的線索整理成可回顧、可調整、可回滾的心智狀態配方。
   - **Mind-Lens** 形塑視角與取捨。
   - **Policy** 確保執行不越過人類定義的邊界。

3. **用會議思考，用記憶保留連續性**

   - **Mindscape Meeting** 處理當下的思考與收斂。
   - **Governed Memory Fabric** 保留 episodic、interface、core、procedural 這幾層長期連續性。

4. **掛接可選的執行層**

   - 你可以保留既有 workflow framework，也可以在需要端到端本地執行時，再把 Mindscape 的 **playbooks / tools / sandboxed runtimes** 接上去。

5. **讓結果可見、可修訂、可演化**

   - 執行會留下 **Execution Trace**、artifact、decision 與 writeback，讓系統能隨時間被檢查與修正。

這個 repo 就是把上述這些元件（governance context、meeting runtime、governed memory、external runtime adapters，以及可選的本地執行層）包成一個可在本機啟動的核心引擎。
...
**重點**：`Project / Flow / Playbooks / Sandbox` 是 **可選的執行模組**，不是使用 Mindscape 心智引擎的前提。
...
如果你使用這套可選的本地執行層，這個 repo 也並不限於「一台機器上的一個使用者」。
