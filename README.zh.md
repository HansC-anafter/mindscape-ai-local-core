...
> **開源、本地優先、人類可治理的 agentic workflow 心智引擎**

[English](README.md) | [中文](README.zh.md)

`mindscape-ai-local-core` 是 **Mindscape AI** 的開源核心 —— 一個**本地優先**、**人類可治理**的 agentic workflow 心智引擎。

它把你的長期意圖、專案主線、創作主題，整理成一個**可治理、可導航的心智空間（mindscape）**，讓 AI 能**圍繞你的長線專案一起思考與行動** —— 並且每個產出都可追溯、可回滾、可人為介入。

你可以用兩種方式來使用它：

- 當成既有 agent stack 裡的 **心智引擎**
- 當成一個帶有可選 `Project / Flow / Playbook / Sandbox` 執行層的 **端到端本地工作空間**

它也原生兼容 **Agent Skills（`SKILL.md`）** 與 **MCP**，因此可以接入既有 agent workflow，而不必先採用本地執行層。
...
> **Mind-Lens 是一個「渲染用的調色盤」** —— 一個由使用者定義的控制介面，把價值觀、美學偏好、工作風格**投射**到 AI 執行中，幫你**在各種工作流中一致地導演輸出**。
...
無論是 **Mindscape Graph（心智圖模式）** 還是 **Workspace 執行（運行模式）**，都在操作同一份 Lens 合約 —— 只是編輯的作用域不同。

---

## 🧠 Mindscape Engine

Mindscape AI 是一個本地優先的引擎：先編譯治理上下文，再進行即時 deliberation，保留長期連續性，最後在需要時把工作路由到本地或外部執行層。
...
- **Governance Context**：由 `Intent`、`Mind-Lens`、`Policy` 構成，定義為什麼做、如何取捨、哪些邊界不能破；需要時也可透過 `Mind-Model VC` 管理這些治理線索的版本變化。
- **Meeting Runtime**：Mindscape Meeting 負責當下的 deliberation、釐清、收斂、派發與閉環。
- **Governed Memory Fabric**：透過 evidence、episodic compression、durable memory、invalidation 與 serving，保留跨輪次的長期連續性。
- **Optional Actuation**：在需要時，把認知結果送往 `Project / Flow / Playbook / Tools / Sandbox` 或外部 runtimes。
- **Reviewable Outputs**：execution trace、artifacts、decisions、writebacks 讓整個系統能被檢查、修正與演化。

這個 repo 就是把上述元件包成一個可在本機啟動的核心引擎，既可以接到既有 agent stack，也可以作為端到端本地工作空間的核心。

---

## 接到你既有的 Agent Stack 裡

> **保留你的 workflow framework，把 Mindscape 接進去補 deliberation 與 governed long-term continuity。**

Mindscape 可以作為既有 agentic workflow stack 的 sidecar，補上多數框架只做得很薄的一段：

- **執行前**：編譯 governance context 與合適的 memory packet
- **執行中**：當任務高歧義、高風險、需要收斂時，升級到 Mindscape Meeting
- **執行後**：整合 evidence、升格 durable memory、讓過期結論失效，並把 continuity 寫回下一輪執行
...
也可以接到自訂 **MCP** 與 **A2A** agents。

| 整合界面 | Mindscape 提供什麼 |
|---------|--------------------|
| **Tool / MCP 層** | deliberation tools、memory packet 準備、reflection 入口 |
| **Workflow hooks** | pre-run context compilation 與 post-run consolidation / writeback |
| **Runtime adapter** | workspace-bound execution context、governance constraints、可追蹤 handoff |
| **Cross-agent continuity** | 跨 session、跨 framework 的 episodic / core / procedural memory |

**重點**：`Project / Flow / Playbooks / Sandbox` 是 **可選的執行模組**，需要端到端本地執行時再掛接即可。

上述框架 logo 皆屬於各自專案，本 README 僅用於表示整合對象。

---

## 場景家族

理解 Mindscape 最好的方式，是看它治理哪一類長期連續性問題。

| 場景家族 | Mindscape 治理的是什麼 | 典型執行界面 |
|---------|------------------------|--------------|
| **品牌與內容治理** | 品牌語氣、故事主線、跨渠道一致性、可回滾的內容決策 | 網站、社群、電子報、客服 |
| **敘事導演與角色演進** | 角色弧線、表演引導、persona 目標、從故事線到執行的連續性 | 影視預演、創作者工作流、面向社群的數位分身 |
| **研究與知識連續性** | 隨 Intent 演化的研究追蹤、證據包、分歧摘要、長文沉澱 | 研究助理、寫作系統、書籍工作流 |
| **練習與教練回圈** | session 記憶、進展基線、回饋安全、留存回圈 | 教練應用、引導式練習系統、垂直陪伴工具 |
| **Agentic Workflow Sidecar** | 跨 run 的 deliberation、reflection、memory packet 路由與 governed writeback | OpenAI Agents SDK、LangGraph、CrewAI、自訂 MCP/A2A 堆疊 |

這些是 **場景家族**。
公開文檔應穩定停留在 governance、deliberation、memory、continuity 與通用整合邊界這幾個層次。

---

## 可選的本地執行層

如果你要端到端在本地執行，這個 repo 也內建一套 **project / playbook** 執行堆疊：
...
如果你使用這套可選的本地執行層，Mindscape 可以把某個領域專家的**看法、做法、與執行邊界**，整理成可分享、可匯入、可重用的模組。

它由幾個可組合的層次構成：

- **視角層**：`Mind-Lens` preset 與 override 決定怎麼理解問題、怎麼取捨、輸出要呈現什麼氣質。
- **角色層**：AI role config 會持久保存角色名稱、對應 playbooks、可用工具，以及 `mindscape_profile_override` 這類角色專屬調整。
- **工作流層**：Playbook 內建 `owner_type`、`visibility`、`shared_with_workspaces` 等作用域規則；真正進入執行的，會是當前工作區可生效的 `effective_playbooks`。
- **可移植層**：portable export（包含 AI roles 與 playbooks），以及 lens preset package / install，讓這些配置可以在不同 profile、workspace、或另一個 local instance 之間移動。

這讓 Mindscape 能把某位專家的**獨特視角與工作方法**整理成可分享的模組。

口語一點地說，這種模組不是在回答「AI 會不會做」，而是在回答下面幾件事：

- **Intent** 決定這次到底要做什麼，例如要審一輪品牌文案、整理一份研究 brief、或推進一段角色弧線。
- **Skills / Tools** 決定可以用哪些能力把事情做出來，例如搜尋、整理、比對、生成、結構化輸出。
- **Mind-Lens** 決定你是用什麼視角、什麼風格、什麼取捨邏輯在做這件事。
- **Meeting Runtime** 負責把當下問題釐清、把選項攤開、把決策收斂成下一步。
- **Governed Memory Fabric** 則把真正值得留下的判斷、偏好、決策依據與未解張力，寫回長期脈絡。

例如同樣是在做一份對外內容：

- `Intent` 可以是「把這次品牌觀點整理成可發佈的初稿」。
- `Skills / Tools` 可以負責蒐集材料、整理提綱、比對版本、生成草稿。
- `Mind-Lens` 會決定這份內容是更像研究編輯那樣嚴謹克制，還是更像品牌主理人那樣鮮明一致。
- `Memory` 會把這次的判斷、證據、取捨與未解張力整理成下一輪可直接接手的長期脈絡；該延續的標準會被送回 context，過時或站不住腳的結論則會被降權、修正或失效。

在產品與應用場景上，這可以長成：

- **品牌主理人視角**：把品牌語氣、審稿標準、內容取捨與跨渠道一致性整理成可重用模組。
- **研究編輯視角**：把證據要求、分歧整理、長文結構與引用紀律整理成可持續套用的研究工作法。
- **教練 / 陪跑視角**：把回饋節奏、進度檢查、反思寫回與安全邊界整理成可重複的陪伴迴圈。
- **敘事導演視角**：把角色弧線、表演取向、故事線連續性與輸出風格整理成可延續的敘事模組。

換句話說，`mindscape-ai-local-core` 已經具備把「專家視角 + 角色配置 + 工作流方法」封裝成可攜模組的基礎骨架。
...
## 🤖 外部執行環境整合 (External Runtimes Integration)

Mindscape 提供**可插拔架構**來整合外部執行環境於其治理層內。

> 外部執行環境（Gemini CLI、OpenClaw、LangGraph 等）提供執行環境。在 Mindscape 架構中，*agent core* 由 Governance Context（Intent + Mind-Lens + Policy）與 Governed Memory Fabric 共同定義；runtime 提供算力和沙箱來執行。

### 核心特點

- **可插拔適配器**：將新的 runtime adapter 放入 `agents/` 目錄，啟動時自動發現
- **統一 API**：所有 runtime 透過 `BaseAgentAdapter` 共享 `execute()` 介面
- **Workspace 沙箱綁定**：所有 runtime 在 Workspace 邊界內的隔離沙箱執行
- **完整可追溯**：所有執行記錄到 Asset Provenance
- **Runtime WebSocket**：即時任務派遣通道（`/ws/agent`），支援認證、多客戶端路由、心跳監控和待處理任務佇列

### 🔒 Workspace 沙箱安全

> **關鍵**：所有外部 runtime 執行現已綁定至 Workspace。
...
Runtime WebSocket 端點（`/ws/agent`）實現後端與 IDE executor runtime 之間的即時雙向通訊：
...
```text
Workspace A (規劃)    →    Workspace C (執行環境)         →    Workspace B (審核)
   定義 Intent                Runtime                       品質檢查
   配置 Lens                  在隔離 sandbox 運行             審核產出
```

這使您可以將一個 Workspace 作為專用的執行環境，而其他 Workspace 處理規劃和審核，全程保持完整治理和審計軌跡。

詳見 [外部 Runtime 架構](./docs/core-architecture/external-agents.md)。
...
* **Governed Memory Fabric（可治理記憶織體）**：長期連續性底座，負責保留證據、形成 episode、升格 durable memory，並把正確的 memory packet 回送到執行環境。
* **Agent（Mindscape 定義）** — Agent 是一個 **AgentSpec** 在 **Agent OS** 上的運行實例：
  - **AgentSpec** = **Agent Core**（(Intent + Mind-Lens + Policy) + Governed Memory Fabric）+ **Actuator**（工具/能力包/允許副作用）
  - **Agent OS** 提供執行合約：觀測/追蹤、回滾、風控 gate、審計與溯源、人類介入的 UI。

  外部整合（Gemini CLI、OpenClaw、LangGraph 等）在架構上視為 **executor runtime**：提供執行環境。Agent 身份留在 AgentSpec。
* **Project（專案）**：把相關的 Intent 與 Playbook 收攏在一起，例如一個產品發佈、一整年的寫書計畫、一個客戶帳號。
* **Playbook**：同時給人看、也給機器跑的工作流腳本（Markdown + YAML frontmatter），是可掛載於 AgentSpec 的策略資產。
* **治理層 (Governance Layer / Agent OS)** — 意圖、視角、信任、資產治理，確保每個 AI 行動都可追溯、可控制。Agent OS = Governance Layer + 執行合約（trace / rollback / risk gate / provenance / HITL）。
* **Port/Adapter 架構**：核心與外部集成的清晰分離，實現本地優先設計並支援可選的雲端擴展。
* **[Governed Memory Fabric](./docs/core-architecture/governed-memory-fabric.md)**：目前對外正式的記憶架構敘事，說明會議運行時、可治理記憶與長期連續性如何協作。
...
在大多數 AI 工具仍停留在「聊天 + 單次工具呼叫」的情境下，`mindscape-ai-local-core` 專注在**長期專案、可視化思考、人類可治理的 AI 工作流**。它是一個帶治理機制的 **AI 工作流作業系統**，讓你可以追溯、版本化、回滾任何 AI 產出。
...
### 核心概念
- [心智空間算法](./docs/mindscape-algorithm.md) - 核心理念與心智空間世界觀
- [Governed Memory Fabric](./docs/core-architecture/governed-memory-fabric.md) - 目前對外正式的記憶架構骨架

### 架構文檔
- [架構文檔](./docs/core-architecture/README.md) - 完整系統架構，包括：
  - Port/Adapter 架構
  - Governed Memory Fabric / Legacy 記憶與意圖架構
  - Mind-Model VC 架構
  - 執行上下文
  - 本地/雲端邊界
