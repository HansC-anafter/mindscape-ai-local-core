...
Critical correction on 2026-05-03: the route-owned object-action/playbook/chat slice is a ledger and runtime-dispatch seed, not the final meeting-led orchestration architecture. The original AOL Runtime Shell design requires object refs, graph guidance, user intent, and pack affordances to enter MeetingEngine as `HandoffIn` / `RequestContract` so MeetingEngine can assemble ActionIntent, TaskIR, dispatch, memory, and review trace. Therefore `route_object_action` and `route_playbook` must be downgraded to legacy/debug or explicit override paths once `route_meeting_orchestration` lands. The blocking P0 implementation plan is `aol-to-meeting-engine-orchestration-bridge-implementation-plan-2026-05-03.md`.
