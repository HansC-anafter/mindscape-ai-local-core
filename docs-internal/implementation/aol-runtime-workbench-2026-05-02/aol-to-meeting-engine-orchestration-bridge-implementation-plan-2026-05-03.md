...
本計劃內所有 IG refs -> PD storyboard E2E 驗收文字，均受 `pd-storyboard-e2e-preflight-ledger-2026-05-05.md` 覆寫。`_021` 與其他 `codex_aol_e2e_ref_*` 紀錄只能證明 Meeting transport、TaskIR lineage、downstream dispatch、artifact DB/file landing；不得再宣稱完成 real IG refs 高品質 storyboard 內容驗收。新增或重跑 E2E 必須先完成 `E2E-PD-PREFLIGHT-000`，並使用 real IG catalog refs，例如 `ref_63601788`、`ref_8849fff0`、`ref_50eb8376`、`ref_6702844a`、`ref_c3f6a15d`、`ref_9ddb375f`、`ref_21f1b00a`、`ref_23953361`。
...
| 原始指令 #2：用多個 IG refs 構思一組 `90s reels` 分鏡 | true | false | `_021` 只作為 legacy transport fixture：manifest verifies `scene_count=9`, `total_duration_sec=90`, selected source refs `codex_aol_e2e_ref_a_20260503` / `codex_aol_e2e_ref_b_20260503`, and `render_profile.profile_id=pd_vertical_reels_storyboard`。依 2026-05-05 ledger，正式驗收必須用 real IG refs、45 scenes、非空 cue map 與逐鏡 LLM judge。 |
| 原始指令 #2：完成分鏡圖製作，且 storyboard image / frame artifacts 入庫入檔 | true | false | `_021` produced a contact-sheet SVG image artifact plus per-scene `scene_manifest.storyboard_frame` entries, but it did not prove requested visual-scope QA for real IG refs or final raster/reels/video output. DB/file evidence remains useful as legacy artifact landing proof only. |
| Pack-owned per-scene review carrier：PD storyboard artifact 必須包含可被 Meeting graph 消費的逐分鏡討論/決策資料結構 | true | true | `_021` proves pack-owned per-scene review carriers: every scene has `meeting_discussion_prompt`, `decision_items`, `review_candidates`, and `approval_state=needs_review`. This is only carrier readiness; it is not accepted as director-thinking guidance or decision-loop E2E closure. |
| 原始指令 #1/#3：導演思維引導工作與逐分鏡決策閉環 E2E。使用者必須能在 AOL/Meeting graph 中選中 storyboard scene，觸發 PD-owned director guidance，看到 thinking prompts / guidance cards / evidence dock / proposal draft，並把 accept/revise/reject/rerun decision 以 decision ledger 或 proposal review artifact 落庫落檔 | true | false | Not covered by `_021`. `_021` only proves `meeting_discussion_prompt` / `decision_items` / `review_candidates` carrier presence. It does not run `pd_director_guidance`, does not persist `DirectorGuidanceState`, does not materialize a director proposal draft, and does not record an accept/revise/reject/rerun decision. This remains an open E2E gate. |
...
### 0.0.2 本次查驗修正：導演思維引導與決策閉環未被 `_021` E2E 覆蓋

2026-05-04 查驗修正：先前把 `_021` 的 per-scene `meeting_discussion_prompt`、`decision_items`、`review_candidates`、`approval_state=needs_review` 描述為 Pack-owned object discussion → MeetingEngine guidance E2E 已完成，這個判定過度。這些欄位只能證明「review carrier readiness」，不能證明使用者原始指令 #1/#3 所要求的「在 pd pack 邊使用 AOL shell 透過 meeting 討論每一個分鏡的引導創作」已完成。

正確 gate 必須獨立驗收：

- `@storyboard_scene:{scene_id}` 或等價 scene ref 從 AOL/Meeting graph 選中並進入 `route_meeting_orchestration`。
- selected guidance metadata 必須指向 `performance_direction/pd_director_guidance` 或等價 PD-owned director guidance playbook。
- 下游必須產生並落庫/落檔 `DirectorGuidanceState`，內容至少包含 `thinking_prompts`、`guidance_cards`、`evidence_dock_state`、`proposal_draft`。
- 對同一 scene 必須形成可追蹤的 director decision：`accept`、`revise`、`reject` 或 `rerun`，並寫入 `decision_ledger`、proposal review artifact 或等價 pack-owned decision record。
- UI 必須能從 Meeting graph/PD storyboard editor 看到該 scene 的導演引導狀態、證據、待決問題、決策結果與下一步。

因此 `_021` 不得再被當成導演思維引導/決策閉環的 E2E 完成證據；它只保留為 storyboard transport、artifact landing、review carrier readiness 的完成證據。
...
2026-05-04 final closure: command `cmd_aol_real_e2e_files_20260504_021_tasklineage` supersedes `_014`, `_018`, `_019`, and `_020` as the legacy transport/artifact-landing record. It returned `status=completed`, `dispatch_status=completed`, `task_ir_id=task_f385ff20d3364399`, downstream execution id `7ba39e58-e19f-4113-b8db-5547558e26bd`, `artifact_landing_status=landed`, three artifact DB ids, and three concrete file paths: contact-sheet SVG, proposal Markdown, and storyboard manifest JSON. DB rows carry `thread_id=0f2463d0-2f22-4016-9b5d-cb3b389eb8d1` and `task_id=task_f385ff20d3364399`; content verification confirms 9 scenes, 90 seconds total, per-scene frame metadata, per-scene discussion/review carriers, and `render_profile.profile_id=pd_vertical_reels_storyboard`. Under the 2026-05-05 ledger, this is not a real IG refs high-quality storyboard acceptance record. Evidence is recorded in `docs-internal/implementation/aol-runtime-workbench-2026-05-02/aol-real-file-e2e-evidence-2026-05-04.md`.
...
- Legacy content verification confirms storyboard id `sb_a75480dadd93`, direction session `ds_f4ae71893782`, command id `cmd_aol_real_e2e_files_20260504_021_tasklineage`, and synthetic source refs `codex_aol_e2e_ref_a_20260503` / `codex_aol_e2e_ref_b_20260503`.
- The old product surface gate passed only for this legacy fixture: `scene_count=9`, `total_duration_sec=90`, `all_have_frames=true`, `all_need_review=true`, `all_have_discussion_prompt=true`, `all_have_decision_items=true`, `all_have_review_candidates=true`, and `render_profile=pd_vertical_reels_storyboard`. Under the 2026-05-05 ledger, this is not a real IG refs high-quality content pass.
