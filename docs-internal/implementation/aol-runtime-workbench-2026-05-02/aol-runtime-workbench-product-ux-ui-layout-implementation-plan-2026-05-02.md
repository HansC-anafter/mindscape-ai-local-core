...
- no pack-specific IG/PD semantics are hard-coded in local-core workbench components
...
3. **Command Dock remains primary**
   - Manual path: open IG References, select a reference, insert mention/template, submit from Command Dock.
   - Expected true: card interaction inserts an `@ig.reference` chip/template; no run starts until the dock command is submitted.
...
5. **Guidance is graph state, not only side-panel text**
   - Manual path: attach a PD scene or IG reference that has pack-owned guidance.
...
8. **Command Dock uses backend command ledger in full mode**
   - Manual path: submit `/stage @ig.reference:ref_123 for @pd.scene:sc01` from the Command Dock and inspect network plus ledger row.
