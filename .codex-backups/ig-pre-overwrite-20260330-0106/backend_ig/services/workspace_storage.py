...
class WorkspaceStorage:
    ...
    def get_references_path(self) -> Path:
        """Get path for IG references directory (pinned assets)."""
        path = self.backend.get_path("references")
        path.mkdir(parents=True, exist_ok=True)
        return path

    def get_reference_account_path(self, source_handle: str) -> Path:
        """Get path for a source account's references directory.

        Args:
            source_handle: Instagram handle (with or without @).
                           Stored as @handle directory.
        """
        handle = source_handle if source_handle.startswith("@") else f"@{source_handle}"
        path = self.get_references_path() / handle
        path.mkdir(parents=True, exist_ok=True)
        return path

    def get_references_unsorted_path(self) -> Path:
        """Get path for unsorted references (source unknown)."""
        path = self.get_references_path() / "_unsorted"
        path.mkdir(parents=True, exist_ok=True)
        return path
    ...
    def from_workspace_path(
        ...
        SECURITY WARNING: This method is deprecated for enterprise/multi-tenant mode.
        Use Workspace.storage_root_ref (managed config) instead.
