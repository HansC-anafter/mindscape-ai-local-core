...
class CompleteWorkflow:
    ...
    def execute_brief(self, brief: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute a brief-driven workflow.

        The brief is an internal contract that can be produced by planning/brand/persona layers.
        This method supports a minimal bridge without changing local-core primitives:

        - If brief contains workflow_name + workflow_steps, execute it directly.
        - If brief contains post_content + post_metadata, execute create_post_workflow.
        """
        brief0 = brief if isinstance(brief, dict) else {}

        workflow_name = brief0.get("workflow_name")
        workflow_steps = brief0.get("workflow_steps")
        initial_context = brief0.get("initial_context")
        if workflow_name and isinstance(workflow_steps, list):
            return self.execute_workflow(
                workflow_name=str(workflow_name),
                workflow_steps=workflow_steps,
                initial_context=initial_context if isinstance(initial_context, dict) else None,
            )

        post_content = brief0.get("post_content")
        post_metadata = brief0.get("post_metadata")
        target_folder = brief0.get("target_folder")
        if isinstance(post_content, str) and isinstance(post_metadata, dict):
            return self.create_post_workflow(
                post_content=post_content,
                post_metadata=post_metadata,
                target_folder=str(target_folder) if target_folder is not None else None,
            )

        return {
            "workflow_name": "execute_brief",
            "status": "failed",
            "error_message": "Invalid brief: expected workflow_name/workflow_steps or post_content/post_metadata",
            "step_results": [],
            "final_context": {"brief_keys": list(brief0.keys())},
            "executed_at": datetime.now().isoformat(),
        }
    ...
    def _evaluate_condition(
        ...
        try:
            ...
            # If variable is not resolved (e.g. "{{post_folder}}"), treat as False quietly.
            if (
                isinstance(resolved_condition, str)
                and resolved_condition.strip().startswith("{{")
                and resolved_condition.strip().endswith("}}")
            ):
                return False
