...
export function KnownSeedsCard(props: {
  seedOptions: FilterOption[];
  onViewAllTargets: () => void;
  onSelectSeedKey: (key: string) => void;
}) {
  const { seedOptions, onViewAllTargets, onSelectSeedKey } = props;

  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Known seeds</h3>
        <button
      ...
      {seedOptions.length === 0 ? (
        <div className="text-sm text-gray-500 dark:text-gray-400">
          No seeds found yet. Run a capture first.
        </div>
      ) : (
