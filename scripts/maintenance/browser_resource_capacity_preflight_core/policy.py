...
def evaluate_capacity(inputs: CapacityInputs) -> dict[str, Any]:
    ...
    byte_capacity = min(inputs.running_count, inputs.required_concurrency)
    ...
    if inputs.eligible_candidate_count < inputs.required_concurrency:
        blockers.append("lock_or_partition_capacity_below_required")
    ...
    if inputs.running_lock_conflict_count > 0:
        blockers.append("running_lock_conflict")
    if inputs.stale_running_count > 0:
        blockers.append("stale_running_tasks")
    ...
    if inputs.mode == "post-resume":
        if inputs.fresh_live_running_count < inputs.required_concurrency:
            blockers.append("fresh_live_running_below_required")
