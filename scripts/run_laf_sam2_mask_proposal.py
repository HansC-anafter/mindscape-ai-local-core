...
def main() -> int:
    ...
    worker = Path(__file__).with_name("laf_sam2_mask_worker.py")
    ...
    command = [str(python_bin), str(worker), "--bbox-json", args.bbox_json]
    if args.image_path:
        ...
        json.dumps(
            {
                ...
                "stderr": stderr or stdout or "SAM2 host worker failed",
            }
        )
    )
    return 0
