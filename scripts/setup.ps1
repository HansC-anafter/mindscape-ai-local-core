...
if ($nodeAvailable) {
    ...
    Write-Host "  WARNING  Node.js not found. Device Node and MCP Gateway require Node.js." -ForegroundColor Yellow
...
if ($ollamaAvailable) {
    ...
    Write-Host "  WARNING  Ollama not found (optional). For local inference without API keys:" -ForegroundColor Yellow
...
if (-not (Test-Path $envFile)) {
    ...
    if (Test-Path $envTemplate) {
        ...
        Write-Host "  WARNING  No .env file found. start.ps1 will use defaults." -ForegroundColor Yellow
    }
} else {
...
if ((Test-Path $deviceNodeDir) -and $nodeAvailable) {
    ...
    Write-Host "  OK Device Node built" -ForegroundColor Green
}
