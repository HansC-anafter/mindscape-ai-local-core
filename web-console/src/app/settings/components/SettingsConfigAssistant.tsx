  ...
  function SettingsConfigAssistant({ currentTab, currentSection, onNavigate }, ref) {
    ...
    const generateWelcomeMessage = useCallback((ctx: SettingsContext | null, tab: SettingsTab): { content: string; actions?: Array<{ label: string; action: string; params?: Record<string, any> }> } => {
      ...
      if (!ctx) {
        switch (tab) {
          case 'basic':
            content = t('configAssistantBasicTab' as any) || 'You are on the Basic Settings page.';
          ...
          case 'packs':
            ...
            break;
          case 'service_status':
            content = t('configAssistantServiceStatusTab' as any) || 'You are on the Service Status page.';
            ...
            break;
          default:
            content = t('configAssistantWelcome' as any) || 'Welcome! I can help you with:';
            suggestions.push(t('configAssistantHelpLLM' as any) || 'Configure LLM API keys');
            suggestions.push(t('configAssistantHelpTools' as any) || 'Connect tools and services');
            suggestions.push(t('configAssistantHelpDiagnose' as any) || 'Diagnose configuration issues');
            suggestions.push(t('configAssistantHelpAnswer' as any) || 'Answer configuration questions');
        }

        if (suggestions.length > 0) {
          content += '\n\n' + (t('configAssistantSuggestions' as any) || 'Suggestions:');
      ...
      switch (currentTab) {
        case 'basic':
          ...
          if (!configSnapshot.backend.openai_configured && !configSnapshot.backend.anthropic_configured) {
            issues.push(t('configAssistantIssueNoLLM' as any) || 'No LLM API keys configured');
            actions.push({
              label: t('configureLLMKeys' as any) || 'Configure LLM Keys',
              action: 'navigate',
              params: { tab: 'basic' }
            });
          }
        ...
        case 'tools':
          ...
          if (configSnapshot.tools.issues.length > 0) {
            issues.push(`${configSnapshot.tools.issues.length} ${t('configAssistantIssueTools' as any) || 'tools not connected'}`);
            actions.push({
              label: t('connectTools' as any) || 'Connect Tools',
              action: 'navigate',
              params: { tab: 'tools' }
            });
          }
        ...
        case 'service_status':
          ...
          if (configSnapshot.services.issues.length > 0) {
            issues.push(`${configSnapshot.services.issues.length} ${t('configAssistantIssueServices' as any) || 'service issues'}`);
          }
          break;
...
Be concise, helpful, and action-oriented.`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          const fallbackMessage: Message = {
            id: (Date.now() + 1).toString(),
            role: 'assistant',
            content: t('configAssistantFallback' as any) || 'The assistant API is not yet available. Based on your current configuration, I can help you navigate to the right settings. What would you like to configure?',
            timestamp: new Date(),
            actions: [
              {
                label: t('basicSettings' as any) || 'Basic Settings',
                action: 'navigate',
                params: { tab: 'basic' }
              },
              {
                label: t('toolsAndIntegrations' as any) || 'Tools & Integrations',
                action: 'navigate',
                params: { tab: 'tools' }
              }
            ]
        ...
        const errorMessage: Message = {
          id: (Date.now() + 1).toString(),
          role: 'assistant',
          content: t('configAssistantError' as any) || 'Sorry, I could not process your request. Please try again later.',
          timestamp: new Date()
        };
    ...
    // External message sending function for Chat-First UX
    const sendMessageExternal = useCallback(async (messageText: string) => {
    ...
    }, [context, currentTab, currentSection, isLoading, buildSystemPrompt]);

    // Expose sendMessage via ref for external components
    useImperativeHandle(ref, () => ({
      sendMessage: sendMessageExternal
    }), [sendMessageExternal]);
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-gray-100 dark:bg-gray-700 rounded-lg px-3 py-2 text-xs text-gray-600 dark:text-gray-300">
                <span className="inline-block animate-pulse">{t('thinking' as any) || 'Thinking...'}</span>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-3 flex-shrink-0">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder={t('configAssistantPlaceholder' as any) || 'Ask about configuration...'}
