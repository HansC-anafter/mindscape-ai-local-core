...
export function UnsplashFingerprintsSettings() {
  ...
  const handleDownloadAndBuild = async () => {
    if (!confirm(t('unsplashFingerprintsConfirmDownload' as any) || 'This will download ~6.5M photos metadata (~2-5GB) and may take 10-30 minutes. Continue?')) {
      return;
    }
    ...
    try {
        ...
        try {
          ...
          if (progressResponse.ok) {
            ...
            if (progressData.status === 'completed') {
              ...
              showNotification('success', t('unsplashFingerprintsSetupSuccess' as any) || 'Fingerprints database setup completed successfully!');
              await fetchStatus();
            } else if (progressData.status === 'failed') {
              setDownloading(false);
              showNotification('error', progressData.error || t('unsplashFingerprintsSetupFailed' as any) || 'Setup failed');
            } else {
              // Continue polling
      ...
      setDownloading(false);
      showNotification('error', error.message || t('unsplashFingerprintsSetupError' as any) || 'Failed to setup fingerprints');
    }
  };
    ...
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
          {t('unsplashFingerprintsTitle' as any) || 'Unsplash Dataset Fingerprints'}
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          {t('unsplashFingerprintsDescription' as any) || 'Enhance Visual Lens extraction quality by downloading and building a fingerprint database from the Unsplash Dataset. This database contains detailed color, keyword, and metadata for millions of photos.'}
        </p>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-sm font-medium text-gray-900 dark:text-gray-100">
            {t('unsplashFingerprintsStatus' as any) || 'Database Status'}
          </h4>
          {status?.has_data && (
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-600 dark:text-gray-400">
              {t('unsplashFingerprintsTotalCount' as any) || 'Total Fingerprints:'}
            </span>
          ...
          {status?.last_updated && (
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">
                {t('unsplashFingerprintsLastUpdated' as any) || 'Last Updated:'}
              </span>
          ...
          {status?.error && !status?.has_data && (
            <div className="text-xs text-gray-500 dark:text-gray-400 italic mt-2">
              {t('unsplashFingerprintsTableNotCreated' as any) || 'Database table not created yet. Click "Download & Build Database" to start.'}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-accent-10 dark:bg-blue-900/20 rounded-lg p-4 border border-accent/30 dark:border-blue-800">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-accent dark:text-blue-300">
            <p className="font-medium mb-1">{t('unsplashFingerprintsInfoTitle' as any) || 'What will be downloaded?'}</p>
            <ul className="list-disc list-inside space-y-1 ml-2">
              <li>{t('unsplashFingerprintsInfo1' as any) || '~25,000 photos metadata (colors, keywords, EXIF data) - Lite version'}</li>
              <li>{t('unsplashFingerprintsInfo2' as any) || 'Requires approx. 200-500MB storage space'}</li>
              <li>{t('unsplashFingerprintsInfo3' as any) || '5-15 minutes processing time'}</li>
              <li>{t('unsplashFingerprintsInfo4' as any) || 'Requires Hugging Face account (free)'}</li>
              {t('unsplashFingerprintsInfo5' as any) && (
                <li className="text-yellow-700 dark:text-yellow-400 font-medium">{t('unsplashFingerprintsInfo5' as any)}</li>
              )}
            </ul>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-yellow-800 dark:text-yellow-300">
                {t('unsplashFingerprintsDownloading' as any) || 'Downloading and building database...'}
              </p>
              {progress && (
              ...
              <p className="text-xs text-yellow-600 dark:text-yellow-500 mt-2 italic">
                {t('unsplashFingerprintsProgressNote' as any) || 'This may take 10-30 minutes. You can close this page and check back later.'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {t('refresh' as any) || 'Refresh Status'}
        </button>
        <button
        ...
        >
          {downloading
            ? (t('unsplashFingerprintsDownloading' as any) || 'Downloading...')
            : (t('unsplashFingerprintsDownloadButton' as any) || 'Download & Build Database')}
        </button>
      </div>
    </div>
  );
