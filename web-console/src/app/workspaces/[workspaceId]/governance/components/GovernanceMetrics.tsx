    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          setData(null);
          setError(t('governanceMetricsCloudOnly' as any));
          return;
        }
        throw new Error(t('failedToLoadMetrics' as any));
      }

      const metricsData: GovernanceMetricsData = await response.json();
      setData(metricsData);
    } catch (err) {
      setData(null);
      setError(err instanceof Error ? err.message : t('failedToLoadMetrics' as any));
    } finally {
      setLoading(false);
    }
  }, [period, t, workspaceId]);
      ...
      {loading ? (
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          ...
          <p className="text-xs text-red-600 dark:text-red-400 mt-2">
            {t('governanceMetricsCloudOnly' as any)}
          </p>
        </div>
      ) : !data ? (
        <div className="text-center py-8 text-secondary dark:text-gray-400">
                    ...
                    <div key={index} className="flex items-center gap-3">
                      ...
                      {item.rejection_count > 0 && (
                        <div className="text-xs text-red-600 dark:text-red-400 w-16 text-right">
                          {t('rejectionsCount' as any, {
                            count: String(item.rejection_count),
                          })}
                        </div>
                      )}
                    </div>
