...
describe('AOLRuntimeShell surface behavior', () => {
  ...
  it('mounts one shared anchor and one shared expansion surface across multiple shells', async () => {
    ...
    expect(screen.getByTestId('aol-runtime-flow-tool-group')).not.toBeNull();
    await waitFor(() => {
      expect(screen.getByTestId('aol-runtime-flow-anchor')).not.toBeDisabled();
    });
    ...
    fireEvent.click(screen.getByTestId('aol-runtime-flow-anchor'));
    await waitFor(() => {
  ...
  it('opens the runtime flow from existing meeting sessions before object selection', async () => {
    render(
      <AOLRuntimeShellProvider workspaceId="ws-global">
        <AOLRuntimeShell
          apiUrl="http://api.test"
