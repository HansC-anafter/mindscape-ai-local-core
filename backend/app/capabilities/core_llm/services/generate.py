...
async def run(
    ...
    **kwargs
    ...
    try:
        if not llm_provider:
            raise ValueError(
                "core_llm.generate requires explicit llm_provider; "
                "implicit provider resolution has been removed"
            )
        if not model_name or not str(model_name).strip():
            raise ValueError(
                "core_llm.generate requires explicit model_name; "
...
def _detect_prompt_language(text: str) -> Optional[str]:
    ...
    if has_chinese:
        ...
        # Traditional characters examples: qing (請), shuo (說), zhe (這)
        # Simplified characters examples: qing (请), shuo (说), zhe (这)
