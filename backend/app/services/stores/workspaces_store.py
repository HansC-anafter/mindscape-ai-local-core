...
class WorkspacesStore(StoreBase):
    ...
    def _row_to_workspace(self, row) -> Workspace:
        ...
        try:
            data_sources = (
                self.deserialize_json(row["data_sources"], None)
                if row["data_sources"]
                else None
            )
        except (KeyError, IndexError):
        ...
        try:
            playbook_auto_execution_config = (
                self.deserialize_json(row["playbook_auto_execution_config"], None)
                if row["playbook_auto_execution_config"]
                else None
            )
        except (KeyError, IndexError):
        ...
        try:
            suggestion_history = (
                self.deserialize_json(row["suggestion_history"], None)
                if row["suggestion_history"]
                else None
            )
        except (KeyError, IndexError):
        ...
        try:
            storage_base_path = (
                row["storage_base_path"] if row["storage_base_path"] else None
            )
        except (KeyError, IndexError):
        ...
        try:
            storage_config = (
                self.deserialize_json(row["storage_config"], None)
        ...
        try:
            playbook_storage_config = (
                self.deserialize_json(row["playbook_storage_config"], None)
                if row["playbook_storage_config"]
                else None
            )
        except (KeyError, IndexError):
        ...
        try:
            expected_artifacts = (
                self.deserialize_json(row["expected_artifacts"], None)
        ...
        try:
            execution_priority = (
                row["execution_priority"] if row["execution_priority"] else "medium"
            )
        except (KeyError, IndexError):
        ...
        try:
            project_assignment_mode_str = (
                row["project_assignment_mode"]
                if row["project_assignment_mode"]
                else "auto_silent"
            )
        ...
        try:
            metadata = (
                self.deserialize_json(row["metadata"], {}) if row["metadata"] else {}
            )
        except (KeyError, IndexError):
            metadata = {}
        ...
        try:
            workspace_type_str = (
                row["workspace_type"] if row["workspace_type"] else "personal"
            )
        ...
        try:
            blueprint_data = (
                self.deserialize_json(row["workspace_blueprint"], None)
                if row["workspace_blueprint"]
                else None
            )
            if blueprint_data:
        ...
        try:
            launch_status_str = (
                row["launch_status"] if row["launch_status"] else "pending"
            )
        ...
        try:
            starter_kit_type = (
                row["starter_kit_type"] if row["starter_kit_type"] else None
            )
        except (KeyError, IndexError):
        ...
        return Workspace(
            ...
            primary_project_id=(
                row["primary_project_id"] if row["primary_project_id"] else None
            ),
            default_playbook_id=(
                row["default_playbook_id"] if row["default_playbook_id"] else None
            ),
