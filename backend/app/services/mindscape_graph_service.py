...
class NodeIdPrefix(str, Enum):
    ...
    ARTIFACT = "artifact"
    MANUAL = "manual"
    REASONING = "reasoning"  # SGR reasoning nodes
...
class EdgeType(str, Enum):
    ...
    SUPPORTS = "supports"  # 0.8 confidence
    CONTRADICTS = "contradicts"  # 0.8 confidence
    DERIVED_FROM = "derived_from"  # 1.0 confidence
...
class EdgeOrigin(str, Enum):
    ...
    DERIVED = "derived"
    USER = "user"
    SGR = "sgr"  # Generated by SGR reasoning middleware
...
class MindscapeGraphService:
    ...
    def derive_from_reasoning_graph(
        ...
        Inserts reasoning nodes (premise, inference, conclusion, etc.)
        and SGR-specific edges (SUPPORTS, CONTRADICTS, DERIVED_FROM)
        into the mindscape graph.

        Args:
            graph: The target MindscapeGraph to add reasoning nodes/edges to
            trace_id: The reasoning trace ID for provenance
            reasoning_graph: Graph dict with 'nodes' and 'edges' keys
        """
        sgr_nodes = reasoning_graph.get("nodes", [])
        sgr_edges = reasoning_graph.get("edges", [])

        # Map SGR node IDs to Mindscape node IDs
        ...
        for sgr_node in sgr_nodes:
            ...
            node = MindscapeNode(
                ...
                label=sgr_node.get("content", "")[:100],  # Truncate for label
        ...
        # Map SGR edge relations to EdgeType
        relation_map = {
    ...
    async def update_overlay(
        ...
        # Manual nodes/edges additions (used by create_manual_node/edge endpoints)
