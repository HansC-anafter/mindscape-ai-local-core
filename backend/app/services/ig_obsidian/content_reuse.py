...
class ContentReuse:
    ...
    def article_to_carousel(
        ...
        Args:
            source_post_path: Path to source article (relative to vault)
            target_folder: Target folder for carousel posts (relative to vault)
        ...
        """
        full_source_path = self.vault_path / source_post_path
        ...
        for i, chunk in enumerate(content_chunks, 1):
            ...
            created_posts.append({
                "slide_number": i,
                "path": str(target_dir.relative_to(self.vault_path) / slide_filename),
    ...
    def carousel_to_reel(
        ...
        Args:
            carousel_posts: List of carousel post paths (relative to vault)
            target_folder: Target folder for reel post (relative to vault)
        ...
        return {
            ...
            "target_folder": target_folder,
            "reel_path": str(target_dir.relative_to(self.vault_path) / reel_filename),
    ...
    def reel_to_stories(
        ...
        Args:
            source_reel_path: Path to source reel post (relative to vault)
            target_folder: Target folder for story posts (relative to vault)
            story_count: Number of stories to create (default: 3)

        Returns:
            Dictionary with created story posts information
        """
        full_source_path = self.vault_path / source_reel_path
        ...
        for i, chunk in enumerate(content_chunks, 1):
            ...
            created_stories.append({
                "story_number": i,
                "path": str(target_dir.relative_to(self.vault_path) / story_filename),
                "content_preview": chunk[:100] + "..." if len(chunk) > 100 else chunk
            })
