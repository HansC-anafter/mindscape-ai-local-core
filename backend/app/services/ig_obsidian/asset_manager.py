...
class AssetManager:
    ...
    def scan_assets(self, post_folder: str) -> Dict[str, Any]:
        """
        ...
        if assets_folder.exists() and assets_folder.is_dir():
            for asset_file in assets_folder.iterdir():
                if asset_file.is_file():
                    asset_info = self._analyze_asset(asset_file, post_slug)
                    if asset_info:
        ...
        return {
            ...
            "post_slug": post_slug,
            "assets_folder": str(assets_folder.relative_to(self.vault_path)) if assets_folder.exists() else None
        }

    def _analyze_asset(self, asset_path: Path, post_slug: Optional[str]) -> Optional[Dict[str, Any]]:
        """
        Analyze asset file
        ...
        try:
            ...
            asset_info = {
                "path": str(asset_path.relative_to(self.vault_path)),
    ...
    def generate_asset_list(self, post_folder: str, post_type: str) -> Dict[str, Any]:
        """
