...
It has been replaced by modular installers:
- MindpackExtractor: Extracts .mindpack files
- ManifestValidator: Validates manifest.yaml
- PlaybookInstaller: Installs playbooks
- RuntimeAssetsInstaller: Installs runtime assets (API, models, services, tools, UI, migrations)
- PostInstallHandler: Handles post-installation tasks

New code should use the modular installers instead of this class.
...
class CapabilityInstaller:
    ...
    def _install_migrations(
        ...
        # Target: alembic_migrations/postgres/versions/ (where Alembic expects migration files)
        ...
        if installed_files:
            if "migrations" not in result.get("installed", {}):
                result.setdefault("installed", {})["migrations"] = []
    ...
    def _install_ui_components(
        ...
        Args:
            ...
            result: Result dict to update
        """
        # Target directory: web-console/src/app/capabilities/{capability_code}/
        # Preserve subdirectory structure (components/, pages/, etc.)
        frontend_dir = self.local_core_root / "web-console" / "src" / "app" / "capabilities"
        target_cap_dir = frontend_dir / capability_code

        # Clean up old installation to prevent leftover files causing conflicts
        if target_cap_dir.exists():
            logger.info(f"Cleaning up old UI installation: {target_cap_dir}")
            shutil.rmtree(target_cap_dir)
        ...
        if source_ui_dir.exists() and source_ui_dir.is_dir():
            ...
            # ui/components/Component.tsx -> capabilities/{code}/components/Component.tsx
            # ui/pages/Page.tsx -> capabilities/{code}/pages/Page.tsx
            for file_path in source_ui_dir.rglob("*"):
                if file_path.is_file():
                    relative_path = file_path.relative_to(source_ui_dir)
                    # Preserve subdirectory structure (components/, pages/, etc.)
                    target_path = target_cap_dir / relative_path
                    target_path.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(file_path, target_path)
                    logger.debug(f"Installed UI file: {relative_path} -> {relative_path}")
                    installed_components.append(str(relative_path))
        ...
        for component_def in ui_components:
            ...
            # Preserve subdirectory structure from component_path
            # e.g., ui/pages/ChapterStudioPage.tsx -> capabilities/{code}/pages/ChapterStudioPage.tsx
            # e.g., ui/components/WorkbenchLayout.tsx -> capabilities/{code}/components/WorkbenchLayout.tsx
            ...
            if len(path_parts) >= 2 and path_parts[0] == 'ui':
                # Preserve subdirectory structure: ui/{subdir}/{file} -> {subdir}/{file}
                subdir = path_parts[1]  # e.g., "pages" or "components"
                filename = path_parts[-1]  # e.g., "ChapterStudioPage.tsx"
                target_subdir = target_cap_dir / subdir
                target_subdir.mkdir(parents=True, exist_ok=True)
                target_path = target_subdir / filename
            else:
                # Fallback: put in components/ directory
                target_path = target_cap_dir / "components" / source_path.name
                target_path.parent.mkdir(parents=True, exist_ok=True)
