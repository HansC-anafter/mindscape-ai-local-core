        ...
        if "tool_call" in parsed_json:
            ...
            if isinstance(result, dict) and "tool_name" in result:
                ...
                if normalized_name != tool_name:
                    ...
                    logger.info(
                        f"Normalized tool name: {tool_name} -> {normalized_name}"
                    )
            return result
            ...
            if alt_key in parsed_json:
                ...
                if isinstance(data, dict):
                    ...
                    if "tool_slot" in data:
                        logger.info(
                            f"Normalized '{alt_key}' to 'tool_call' (tool_slot format)"
                        )
                        return {
                    ...
                    if "tool_name" in data:
                        logger.info(
                            f"Normalized '{alt_key}' to 'tool_call' (tool_name format)"
                        )
                        # Normalize tool name
                ...
                # e.g., {"tool_code": "filesystem_write_file('path', 'content')"}
                    ...
                    logger.info(
                        f"Found Python-style tool call in '{alt_key}' string, attempting to parse"
                    )
                    parsed_calls = self._parse_python_style_tool_call(data)
                    if parsed_calls:
                        logger.info(
                            f"Successfully parsed Python-style tool call from '{alt_key}': {parsed_calls[0].get('tool_name')}"
                        )
        ...
        for alias, canonical_name in tool_aliases.items():
            ...
            for args_str in matches:
                ...
                if parameters:
                    ...
                    logger.info(
                        f"Parsed Python-style tool call: {alias} -> {canonical_name}({parameters})"
                    )
        ...
        try:
            ...
            if parsed_json:
                ...
                if normalized and isinstance(normalized, dict):
                    ...
                    if "tool_slot" in normalized:
                        tool_calls.append(
                            {
                                ...
                                "parameters": normalized.get(
                                    "parameters", normalized.get("args", {})
                                ),
                            }
                        )
                        logger.info(
                            f"Parsed 1 tool call (normalized, slot): {normalized.get('tool_slot')}"
                        )
                        return tool_calls
                        ...
                        logger.info(
                            f"Parsed 1 tool call (normalized, name): {normalized.get('tool_name')}"
                        )
                        return tool_calls
                ...
                if "tool_call" in parsed_json:
                    ...
                    if isinstance(tool_call_data, dict):
                        if "tool_slot" in tool_call_data:
                            tool_calls.append(
                                {
                                    ...
                                    "parameters": tool_call_data.get(
                                        "parameters", tool_call_data.get("args", {})
                                    ),
                                }
                            )
                            logger.info(
                                f"Parsed 1 tool call from JSON (slot): {tool_call_data.get('tool_slot')}"
                            )
                            return tool_calls
                            ...
                            logger.info(
                                f"Parsed 1 tool call from JSON (name): {tool_call_data.get('tool_name')}"
                            )
                            return tool_calls

                # Check if it's a direct tool call format: {"tool_slot": "..."} or {"tool_name": "..."}
                if "tool_slot" in parsed_json and isinstance(
                    parsed_json.get("tool_slot"), str
                ):
                        ...
                        tool_calls.append(
                            {
                                ...
                                "parameters": parsed_json.get(
                                    "parameters", parsed_json.get("args", {})
                                ),
                            }
                        )
                        logger.info(
                            f"Parsed 1 tool call (direct format, slot): {parsed_json.get('tool_slot')}"
                        )
                        return tool_calls
                elif "tool_name" in parsed_json and isinstance(
                    parsed_json.get("tool_name"), str
                ):
                    # Skip if it looks like structured output
                    if not any(
                        key in parsed_json
                        ...
                        logger.info(
                            f"Parsed 1 tool call (direct format, name): {parsed_json.get('tool_name')}"
                        )
                        return tool_calls
            ...
            if parsed_array and isinstance(parsed_array, list):
                for item in parsed_array:
                    if isinstance(item, dict):
                        ...
                        elif "tool_slot" in item and isinstance(
                            item.get("tool_slot"), str
                        ):
                            tool_calls.append(
                                {
                        ...
                        elif "tool_name" in item and isinstance(
                ...
                if tool_calls:
                    logger.info(
                        f"Parsed {len(tool_calls)} tool call(s) from JSON array"
                    )
                    return tool_calls
            ...
            for match in matches:
                ...
                if parsed:
                    ...
                    elif "tool_slot" in parsed and isinstance(
                        parsed.get("tool_slot"), str
                    ):
                        if not any(
                            key in parsed
                            for key in [
                    ...
                    elif "tool_name" in parsed and isinstance(
                        parsed.get("tool_name"), str
                    ):
                        if not any(
                            key in parsed
                            for key in [
            ...
            if tool_calls:
                logger.info(
                    f"Parsed {len(tool_calls)} tool call(s) from markdown code blocks"
                )
                return tool_calls
            ...
            if python_calls:
                logger.info(
                    f"Parsed {len(python_calls)} tool call(s) from Python-style syntax (fallback)"
                )
                return python_calls

        except Exception as e:
            logger.warning(
                f"Failed to parse tool calls from response: {e}", exc_info=True
            )
