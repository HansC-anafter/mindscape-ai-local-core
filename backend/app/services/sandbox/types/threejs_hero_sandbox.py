"""
Three.js Hero Sandbox implementation

Manages Three.js visual scenes with code components, supporting visual selection
...
class ThreeJSHeroSandbox(BaseSandbox):
    """
    Sandbox for Three.js hero scenes

    Manages:
    - Three.js component files (Component.tsx)
    - HTML preview files (index.html)
    - Visual selection metadata
    - Scene configuration
    """
    ...
    def __init__(
        ...
        self.sandbox_type = "threejs_hero"
    ...
    async def get_change_summary(
        ...
        for path in modified:
            if "Component.tsx" in path or "index.html" in path:
                component_changes.append(path)

        if component_changes:
            summary_parts.append(f"Updated components: {', '.join(component_changes)}")

        return "; ".join(summary_parts) if summary_parts else "No changes"

    async def validate(self) -> Dict[str, Any]:
        """Validate Three.js hero sandbox structure"""
        errors = []
        warnings = []
        ...
        has_component = any("Component.tsx" in f for f in file_paths)
        if not has_component:
            warnings.append("No Component.tsx file found")
