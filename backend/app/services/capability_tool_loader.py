...
class CapabilityToolWrapper(MindscapeTool):
    ...
    def __init__(
        ...
        metadata = create_simple_tool_metadata(
            name=tool_name,
            description=tool_info.get('description', f"{capability_code} tool: {tool_name}"),
...
def load_capability_tool(
    ...
    try:
        ...
        if ':' not in backend:
            logger.warning(f"Invalid backend format for {capability_code}.{tool_name}: {backend}")
            return None
        ...
        try:
            ...
            if module_path.startswith('app.capabilities.'):
                ...
                try:
                    ...
                    if len(parts) >= 3 and parts[2]:  # app.capabilities.{capability_code}
                        capability_code = parts[2]
                        # Check if directory uses hyphen instead of underscore
                        capability_dir = capability_dir if 'capability_dir' in locals() else None
                ...
                app_module_path = module_path.replace('capabilities.', 'app.capabilities.', 1)
                try:
            ...
            logger.warning(f"Failed to import module {module_path} for tool {capability_code}.{tool_name}: {e}")
            return None
        except Exception as e:
            logger.error(f"Failed to load tool {capability_code}.{tool_name}: {e}", exc_info=True)
            return None

    except Exception as e:
        logger.error(f"Error loading capability tool {capability_code}.{tool_name}: {e}", exc_info=True)
        return None
