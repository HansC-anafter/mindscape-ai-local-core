...
Phase 0 implementation: provides ``process_completion()`` as the single
entry-point that all result paths (workflow executor, REST endpoints,
WebSocket message handlers, webhook) should delegate to.
...
3.  (Phase 2) Produces post-landing provenance sidecar via ``PackDispatchAdapter``.
4.  (Planned) Runs ``AcceptanceEvaluator`` and emits governance events.
"""
...
class GovernanceEngine:
    ...
    - Provides a stable API surface for Phase 3/4 extensions
      (AcceptanceEvaluator, provenance tracking, event emission).
    """
    ...
    def process_completion(
        ...
        *,
        ...
        hook surface for Phase 3/4 governance extensions.
        ...
        # --- Phase 0: Delegate to existing landing service (raw data) ---
        ...
        # --- Phase 2: Post-landing provenance sidecar ---
        parsed_output = None
        ...
        # --- Phase 3-2A: AcceptanceEvaluator ---
    ...
    def _resolve_acceptance_tests(self, execution_id: str) -> Optional[List[str]]:
        ...
        """
        try:
            governance = self._resolve_governance_payload(execution_id)
    ...
    def _resolve_governance_payload(self, execution_id: str) -> Optional[Dict[str, Any]]:
        ...
        top_level = extract_governance_payload({"governance": ctx.get("governance")})
        if top_level:
            return top_level

        execution_metadata = ctx.get("execution_metadata")
    ...
    def _sync_correctness_signals(
        ...
        *,
        ...
        acceptance_pass_rate = self._calculate_acceptance_pass_rate(eval_summary)
        ...
        try:
            session = self.meeting_session_store.get_by_id(meeting_session_id)
            if session:
    ...
    def _trigger_follow_up(
        ...
        *,
        ...
        # Resolve current remediation round, acceptance_tests, and task_ir_id
        current_round = 0
        ...
        try:
            ...
            if task:
                ctx = getattr(task, "execution_context", None) or {}
                current_round = ctx.get("remediation_round", 0)
                task_ir_id = ctx.get("task_ir_id")
        ...
        if decision.should_follow_up and decision.follow_up_context:
            # Enrich context with task_ir_id for proper registry semantics
    ...
    def _create_follow_up_task(
        ...
        *,
        ...
        try:
            # Idempotency guard — use task_ir_id (not execution_id) for registry
            ...
            original_exec_id = follow_up_context.get("original_execution_id", "")

            # Bug fix: message_id is required — use original or synthesize
            message_id = (
                follow_up_context.get("original_message_id")
                or f"remediation-{original_exec_id}-r{remediation_round}"
            )
