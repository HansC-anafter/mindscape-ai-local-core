...
class DispatchOrchestrator:
    ...
    def __init__(
        ...
        # Phase 0: Optional idempotency registry (fail-open if None)
        self._handoff_registry_store = handoff_registry_store

        # Phase 2: Spec-aware dispatch adapter (graceful degradation if None)
    ...
    async def _dispatch_phase(
        ...
        # --- Phase 0: SSOT Idempotency Guard ---
        # Must run BEFORE mark_dispatched() to keep attempt state clean.
        if self._handoff_registry_store:
            ...
            if not registered:
                ...
                logger.warning(
                    "Phase %s dispatch blocked by idempotency guard (key=%s)",
                    phase.id,
    ...
    async def _launch_playbook(
        ...
        # Phase 2: Spec-aware field mapping via PackDispatchAdapter
