"""Declarative role-profile planner lane compilation."""
...
from backend.app.services.orchestration.meeting.planner_contract_execution.tool_plan_compiler_common import (
    ...
    flag_enabled,
    has_polling_hint,
    is_world_memory_write,
    optional_string,
    pack_enabled_for_role_profiles,
    string_list,
)
...
class DeclarativePlannerLaneCompiler:
    """Compile the feature-flagged declarative role-profile lane."""

    def __init__(
        self,
        *,
        registry: PlannerContractManifestRegistry,
        role_profile_resolver: MeetingRoleProfileResolver,
    ) -> None:
        self.registry = registry
        self.role_profile_resolver = role_profile_resolver
    ...
    def compile_if_enabled(
        ...
        *,
        ...
        if not (
            flag_enabled("MEETING_ROLE_PROFILES_ENABLED")
            and flag_enabled("DECLARATIVE_PLANNER_LANE_ENABLED")
            and pack_enabled_for_role_profiles(pack_id)
        ):
            return None
        ...
        for category in categories:
            ...
            for raw_step in raw_steps:
                ...
                depends_on = [
                    ...
                    for dependency in string_list(raw_step.get("depends_on"))
                ...
                step = build_step(
                    ...
                    meeting_role_profile_code=selected_profile.code,
                    meeting_lane_code=selected_profile.meeting_lane_code,
                    pack_role_name=self._pack_role_name(raw_step, selected_profile),
                    resource_budget_class=optional_string(
    ...
    def _validate_tool(
        ...
        *,
        ...
        if has_polling_hint(contract) or has_polling_hint(hints):
            raise ValueError(
                f"Declarative planner lane step {step_code} must not declare polling"
            )
        if is_world_memory_write(resource_kind=resource_kind, effect=effect):
            raise ValueError(
    ...
    def _pack_role_name(
        ...
        explicit = optional_string(raw_step.get("pack_role_name"))
        if explicit:
            return explicit
        slot = optional_string(raw_step.get("slot"))
        if slot:
            overrides = selected_profile.slot_overrides.get(slot)
            if isinstance(overrides, dict):
                return optional_string(overrides.get("pack_role_name"))
        return None
