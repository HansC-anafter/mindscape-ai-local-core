...
async def detect_and_create_project_if_needed(
    ...
    if existing_project_id:
        logger.debug(f"[ProjectCreation] Using existing project_id: {existing_project_id}")
    ...
    # Check workspace primary_project_id first
    project_id = workspace.primary_project_id
    if project_id:
        logger.debug(f"[ProjectCreation] Using workspace primary_project_id: {project_id}")
        return project_id, None

    # No existing project, detect if we should create one
    logger.info(f"[ProjectCreation] No project_id, checking if we need to create a project...")
    ...
    if not project_suggestion or project_suggestion.mode != "project":
        logger.debug(f"[ProjectCreation] No project detected (mode={project_suggestion.mode if project_suggestion else None})")
    ...
    if duplicate_project:
        logger.info(f"[ProjectCreation] Using existing duplicate project: {duplicate_project.id}")
    ...
    if confidence >= HIGH_CONFIDENCE_THRESHOLD:
        should_create = True
        logger.info(f"[ProjectCreation] High confidence ({confidence:.2f}), creating project directly")
    elif create_on_medium_confidence and confidence >= LOW_CONFIDENCE_THRESHOLD:
        should_create = True
        logger.info(f"[ProjectCreation] Medium confidence ({confidence:.2f}), creating project (create_on_medium_confidence=True)")

    if should_create:
        # Ensure required fields are present
        if not project_suggestion.project_type or not project_suggestion.project_title:
            logger.warning("[ProjectCreation] Project suggestion missing required fields, skipping creation")
            return None, project_suggestion
        ...
        logger.info(f"[ProjectCreation] Created new project: {project.id} (confidence: {confidence:.2f})")
        ...
        if not workspace.primary_project_id:
            ...
            logger.info(f"[ProjectCreation] Updated workspace primary_project_id to {project.id}")

        return project.id, project_suggestion
    else:
        logger.debug(f"[ProjectCreation] Confidence ({confidence:.2f}) too low, not creating project")
        return None, project_suggestion
