...
class IntentExtractor:
    ...
    async def extract_and_create_timeline_item(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Intent extractor: failed to build context: {ctx_err}",
                    exc_info=True,
                )

            # Extract intents/themes using IntentRegistryPort
            logger.info(
                f"Intent extractor: Calling intent registry with message and context"
            )
            ...
            logger.info(
                f"Intent extractor: Registry returned: {resolution_result.intents} intents, {resolution_result.themes} themes"
            )
            print(
                f"Intent extractor: Registry returned: {len(resolution_result.intents)} intents, {len(resolution_result.themes)} themes",
                file=sys.stderr,
            )
            ...
            for intent_item in intents_list[:5]:
                try:
                    if isinstance(intent_item, dict):
                        ...
                        intent_confidence = intent_item.get(
                    ...
                    if intent_label and len(intent_label.strip()) > 0:
                        candidate_tag = IntentTag(
                            ...
                            confidence=(
                                float(intent_confidence) if intent_confidence else None
                            ),
                        ...
                        logger.info(
                            f"Created candidate IntentTag {candidate_tag.id}: {intent_label[:50]}"
                        )
            ...
            if not intents_list and not themes_list:
                ...
                print(
                    f"Intent extractor: No intents or themes extracted, returning None",
                    file=sys.stderr,
                )
                return None
            ...
            summary = (
                " | ".join(summary_parts)
                if summary_parts
                else "LLM extracted intents and themes"
            )
            ...
            if auto_exec_config and "intent_extraction" in auto_exec_config:
                ...
                if auto_execute_enabled and llm_confidence >= confidence_threshold:
                    ...
                    logger.info(
                        f"IntentExtractor: Intent extraction meets auto-exec threshold (confidence={llm_confidence:.2f} >= {confidence_threshold:.2f})"
                    )
                else:
                    logger.info(
                        f"IntentExtractor: Intent extraction does not meet auto-exec threshold (confidence={llm_confidence:.2f} < {confidence_threshold:.2f})"
                    )
            ...
            if should_auto_execute:
                ...
                timeline_item = TimelineItem(
                    ...
                    summary=self.i18n.t(
                        ...
                        count=intents_added,
                        default=(
                            f"Auto-added {intents_added} intent(s) from message"
                            if intents_added > 0
    ...
    async def update_event_metadata(
        ...
        try:
            ...
            logger.debug(
                f"Updated event {event_id} metadata with extracted intents/themes"
            )
            return True
