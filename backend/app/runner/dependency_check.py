...
class DependencyChecker:
    ...
    async def _check_mlx(self) -> tuple[bool, Optional[str]]:
        """Check MLX liveness with a lightweight HTTP probe.

        A bare TCP connect is not enough for MLX: the port can remain open
        while the actual server is wedged or an orphaned child keeps the
        listener alive. `/v1/models` is the cheapest endpoint that proves the
        service is actually answering requests.
        """
        ...
        try:
            ...
            try:
                ...
                pass
            if status_line.startswith((b"HTTP/1.1 200", b"HTTP/1.0 200")):
                return True, None
            if self._mlx_watchdog_is_fresh():
                return True, None
            return False, status_line.decode("ascii", errors="replace").strip() or "mlx unhealthy"
        except TimeoutError:
            if self._mlx_watchdog_is_fresh():
                return True, None
            return False, "mlx health timeout"
    ...
    def _mlx_watchdog_is_fresh(self) -> bool:
        try:
            ...
            phase_age = max(0.0, now - (phase_entered_at or started_at)) if (phase_entered_at or started_at) else request_age
