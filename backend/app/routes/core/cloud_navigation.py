...
from ...services.capability_api_loader import CapabilityAPILoader
import yaml
...
def load_capability_manifest(capability_code: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        remote_capabilities_dir_str = settings_store.get(
            "remote_capabilities_dir", default=""
        )

        if remote_capabilities_dir_str:
            cloud_capabilities_dir = Path(remote_capabilities_dir_str)
        elif (
            api_loader.remote_capabilities_dir
            and api_loader.remote_capabilities_dir.exists()
        ):
        ...
        if not cloud_capabilities_dir.exists():
            logger.debug(
                f"Remote capabilities directory does not exist: {cloud_capabilities_dir}"
            )
            return None
...
async def redirect_to_ui_component(
    ...
    """
    try:
        route_template = get_ui_component_route(
            capability_code, artifact_type, playbook_code
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to redirect to UI component"
        )
