...
try:
    ...
    def _migrate_cloud_settings(settings_store: SystemSettingsStore):
        ...
        try:
            ...
            if old_api_url and old_license_key and old_enabled:
                ...
                logger.info(
                    "Migrating cloud settings from old format to new cloud_providers array"
                )
    ...
    try:
        ...
        if providers_config:
            logger.info(
                f"Loading {len(providers_config)} cloud providers from settings"
            )
            ...
            for provider_config in providers_config:
                ...
                try:
                    if provider_type == "generic_http":
                        ...
                        if not auth_config:
                            logger.warning(
                                f"Provider {provider_id}: Missing 'auth' configuration, skipping"
                            )
                            continue
                        ...
                        logger.warning(
                            f"Unknown provider type: {provider_type}, skipping"
                        )
                        continue

                    cloud_extension_manager.register_provider(provider)
                    logger.info(
                        f"Registered cloud provider: {provider_id} ({provider_type})"
                    )

                except Exception as e:
                    logger.error(
                        f"Failed to register provider {provider_id}: {e}", exc_info=True
                    )
        else:
        ...
        logger.warning(
            f"Failed to load cloud providers from settings: {e}", exc_info=True
        )

except ImportError:
    logger.debug(
        "Cloud Extension Manager not available (httpx not installed or module not found)"
    )
except Exception as e:
...
async def list_playbooks(
    ...
    locale: str = Query(
        "zh-TW",
        description="[DEPRECATED] Language locale. Use target_language instead.",
    ),
    target_language: Optional[str] = Query(
        None,
        description="Target language for filtering (e.g., 'zh-TW', 'en'). Playbooks are language-neutral by default.",
    ),
    scope: str = Query("system", description="system|user|all"),
    onboarding_task: Optional[str] = Query(
        None, description="Filter by onboarding task"
    ),
    uses_tool: Optional[str] = Query(
        None,
        description="Filter playbooks that require this tool (e.g., 'wordpress', 'canva')",
    ),
    profile_id: str = Query(
        "default-user", description="User profile for personalization"
    ),
    workspace_id: Optional[str] = Query(
        None, description="Filter playbooks used in this workspace"
    ),
    filter: Optional[str] = Query(
        None, description="Filter type: favorites|recent|created_by_me|ready_to_run"
    ),
):
    """
    ...
    try:
        ...
        all_playbook_metadata = await playbook_service.list_playbooks(
            locale=None, tags=tag_list
        )

        file_playbooks = []
        for metadata in all_playbook_metadata:
            playbook = Playbook(metadata=metadata, sop_content="")
        ...
        for playbook in file_playbooks:
            ...
            # If preference specified, prefer matching locale
                ...
                if (
                    playbook_locale == preferred_locale
                    and playbook_dict[code].metadata.locale != preferred_locale
                ):
                    playbook_dict[code] = playbook
                # If current matches preferred locale and new doesn't, keep current
                elif (
                    playbook_dict[code].metadata.locale == preferred_locale
                    and playbook_locale != preferred_locale
                ):
                    continue
                # If neither matches preference, prefer zh-TW > en > ja
                    ...
                    if locale_priority.get(playbook_locale, 0) > locale_priority.get(
                        playbook_dict[code].metadata.locale, 0
                    ):
                        playbook_dict[code] = playbook
        ...
        if tag_list:
            playbooks = [
                p for p in playbooks if any(tag in p.metadata.tags for tag in tag_list)
            ]

        # Filter by onboarding_task
        if onboarding_task:
            playbooks = [
                p for p in playbooks if p.metadata.onboarding_task == onboarding_task
            ]

        # Filter by uses_tool (playbooks that require this tool)
        if uses_tool:
            playbooks = [
                p for p in playbooks if uses_tool in (p.metadata.required_tools or [])
            ]
        ...
        if workspace_id:
            ...
            def _query_workspace_playbooks(ws_id):
            ...
            workspace_playbook_codes.update(
                await asyncio.to_thread(_query_workspace_playbooks, workspace_id)
            )

            # Get pinned playbooks for this workspace
            try:
                pinned_playbooks = await asyncio.to_thread(
                    pinned_playbooks_store.list_pinned_playbooks, workspace_id
                )
                workspace_playbook_codes.update(
                    {pb["playbook_code"] for pb in pinned_playbooks}
                )
            except Exception as e:
                logger.warning(
            ...
            playbooks = [
                p
                for p in playbooks
                if p.metadata.playbook_code in workspace_playbook_codes
            ]
        ...
        if playbook_codes:
            ...
            def _query_workspace_usage(codes):
                with executions_store.get_connection() as conn:
                    cursor = conn.cursor()
            ...
            workspace_usage_counts = await asyncio.to_thread(
                _query_workspace_usage, playbook_codes
            )
        ...
        if playbook_codes:
            ...
            def _query_pinned_workspaces(codes):
                result = {}
                for code in codes:
                    result[code] = (
                        pinned_playbooks_store.get_pinned_workspaces_for_playbook(
                            code, limit=3
                        )
                    )
                return result

            pinned_workspaces_map = await asyncio.to_thread(
                _query_pinned_workspaces, playbook_codes
            )

        # Apply filter parameter
        if filter == "favorites":
            # Filter by favorites
            favorite_codes = await asyncio.to_thread(
                mindscape_store.user_playbook_meta.list_favorites, profile_id
            )
            playbooks = [
                p for p in playbooks if p.metadata.playbook_code in favorite_codes
            ]
        elif filter == "recent":
            # Filter by recent usage
            recent_codes = await asyncio.to_thread(
                mindscape_store.user_playbook_meta.list_recent, profile_id, limit=50
            )
            playbooks = [
                p for p in playbooks if p.metadata.playbook_code in recent_codes
            ]
        ...
        for playbook in playbooks:
            ...
            result = {
                ...
                "kind": (
                    playbook.metadata.kind.value if playbook.metadata.kind else None
                ),
                "capability_code": playbook.metadata.capability_code,
                "user_meta": user_meta or {"favorite": False, "use_count": 0},
                "has_personal_variant": has_personal_variant,
                "default_variant_name": (
                    default_variant.get("variant_name") if default_variant else None
                ),
...
async def get_playbook_variants(
    ...
    try:
        variants = await asyncio.to_thread(
            store.list_personalized_variants,
            profile_id,
            playbook_code,
            active_only=True,
        )
...
async def discover_playbook(request: Dict[str, Any]):
    """
    ...
    try:
        ...
        all_playbooks = [
            Playbook(metadata=m, sop_content="") for m in all_playbook_metadata
        ]
        ...
        if capability_code:
            all_playbooks = [
                ...
                if p.metadata.capability_code == capability_code
            ]
        ...
        if workspace_id:
            ...
            try:
                ...
                def _query_exec_playbooks(ws_id):
                    with executions_store.get_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            """
                ...
                workspace_execution_codes = await asyncio.to_thread(
                    _query_exec_playbooks, workspace_id
                )
            except Exception as e:
                logger.warning(
                    f"Failed to get execution playbooks for workspace {workspace_id}: {e}"
                )
        ...
        for playbook in all_playbooks:
            ...
            if (
                ...
                or any(query_lower in tag for tag in playbook.metadata.tags or [])
            ):
        ...
        return {"suggestion": suggestion, "recommended_playbooks": matched_playbooks}
    except Exception as e:
...
async def get_playbook(
    ...
    target_language: Optional[str] = Query(
        None,
        description="Target language for content (e.g., 'zh-TW', 'en'). Playbooks are language-neutral.",
    ),
    ...
    try:
        ...
        playbook = await playbook_service.get_playbook(
            playbook_code=base_code, locale=preferred_locale
        )
            ...
            for intent_id in intent_ids:
                try:
                    intent = await asyncio.to_thread(
                        mindscape_store.get_intent, intent_id
                    )
                    if intent:
                        associated_intents.append(
                            {
                                ...
                                "title": intent.title,
                                "status": (
                                    intent.status.value if intent.status else "active"
                                ),
                                "priority": (
                    ...
                    associated_intents.append(
                        {"intent_id": intent_id, "title": f"Intent {intent_id}"}
                    )
        ...
        try:
            ...
            for exec_id in active_execution_ids:
                try:
                    ...
                    if (
                        conv_manager
                        and conv_manager.playbook.metadata.playbook_code
                        == playbook_code
                    ):
            ...
            events = await asyncio.to_thread(
                mindscape_store.list_events, profile_id=profile_id, limit=50
            )
            ...
            for event in events:
                ...
                if event.channel == "playbook" and (
                    payload.get("playbook_code") == playbook_code
                    or event.metadata
                    and event.metadata.get("playbook_code") == playbook_code
                ):
        ...
        return {
            "metadata": {
                ...
                "capability_code": (
                    playbook.metadata.capability_code
                    if hasattr(playbook.metadata, "capability_code")
                    else None
                ),
            },
...
async def get_playbook_usage_stats(
    ...
    try:
        stats = await asyncio.to_thread(
            executions_store.get_playbook_workspace_stats, playbook_code
        )
        return stats
    except Exception as e:
        logger.error(
            f"Failed to get usage stats for playbook {playbook_code}: {e}",
            exc_info=True,
        )
        raise HTTPException(
...
async def get_playbook_ui_layout(
    ...
    try:
        ...
        playbook = await playbook_service.get_playbook(
            playbook_code=base_code, locale="zh-TW"
        )
        ...
        if (
            playbook
            and hasattr(playbook, "metadata")
            and hasattr(playbook.metadata, "capability_code")
        ):
            capability_code = playbook.metadata.capability_code
        ...
        playbook_json = PlaybookJsonLoader.load_playbook_json(
            playbook_code=base_code, capability_code=capability_code
        )
        ...
        if playbook_json:
            ...
            # Also check if it's a dict (when loaded as dict)
                ...
                ui_layout = playbook_json.get("ui_layout") or playbook_json.get(
                    "uiLayout"
                )
            # Try accessing via model_dump if it's a Pydantic model
                ...
                try:
                    ...
                    ui_layout = playbook_dict.get("ui_layout") or playbook_dict.get(
                        "uiLayout"
                    )
                except Exception as e:
        ...
        if not ui_layout and capability_code:
            ui_layout = await _load_ui_layout_from_capability(
                capability_code, base_code
            )
        ...
        if not ui_layout:
            ...
            if len(parts) > 1:
                ...
                ui_layout = await _load_ui_layout_from_capability(
                    potential_capability_code, base_code
                )
        ...
        return {
            ...
            "version": (
                ...
                if playbook and hasattr(playbook, "metadata")
                else "1.0.0"
            ),
        }
    except HTTPException:
        ...
        logger.error(
            f"Failed to load UI layout for {playbook_code}: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")


async def _load_ui_layout_from_capability(
    capability_code: str, playbook_code: str
    ...
    try:
        ...
        logger.warning(
            f"Failed to load UI layout from capability {capability_code}: {e}"
        )
        return None
...
async def update_playbook(
    ...
    if request.user_notes is not None:
        ...
        raise HTTPException(
            status_code=501,
            detail="Update playbook not yet implemented in PlaybookService",
        )
    if not updated:
        raise HTTPException(status_code=404, detail="Playbook not found")
...
async def associate_intent_playbook(
    ...
    try:
        association = await asyncio.to_thread(
            store.associate_intent_playbook, intent_id, playbook_code
        )
        return {
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create association: {str(e)}"
        )
...
async def remove_intent_playbook_association(
    ...
    success = await asyncio.to_thread(
        store.remove_intent_playbook_association, intent_id, playbook_code
    )
    if not success:
...
async def get_intent_playbooks(intent_id: str = Path(..., description="Intent ID")):
    ...
    playbook_codes = await asyncio.to_thread(store.get_intent_playbooks, intent_id)
...
async def check_playbook_tools(
    ...
    try:
        ...
        if not playbook:
            playbook = await asyncio.to_thread(store.get_playbook, playbook_code)
        if not playbook:
            raise HTTPException(
...
async def run_playbook_smoke_test(
    playbook_code: str,
    profile_id: str = Query("test-user", description="Profile ID for testing"),
    use_uploaded_files: bool = Query(
        False, description="Use files uploaded via upload-files endpoint"
    ),
):
    """
    ...
    try:
        ...
        try:
            if playbook_code == "pdf_ocr_processing":
                ...
                from tests.test_playbook_yt_script_generation import (
                    TestYtScriptGeneration,
                )

                test_class = TestYtScriptGeneration
            elif playbook_code == "yearly_book_content_save":
                from tests.test_playbook_yearly_book_content_save import (
                    TestYearlyBookContentSave,
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to import test class: {str(e)}"
            )
        except Exception as e:
            logger.error(f"Test execution failed: {e}", exc_info=True)
            raise HTTPException(
                status_code=500, detail=f"Test execution failed: {str(e)}"
            )
