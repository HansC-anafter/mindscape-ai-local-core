...
async def post_quality_voice_through_legacy_xtts_path(
    payload: QualityVoiceSynthesisRequest,
) -> Response:
    """Keep the public path stable while selecting only the Qwen provider."""

    try:
        result = await synthesize_quality_voice_audio(payload)
    except QualityVoiceUnavailable as exc:
