...
async def check_file_exists(
    path: str = Query(..., description="File path to check (relative to /app or absolute)")
):
    """
...
async def get_browser_profile_status(
    ...
    profile_path: Optional[str] = Query(None, description="Optional profile path to check (absolute or under /app)")
):
    """
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT host_key, name, expires_utc, is_persistent FROM cookies WHERE host_key LIKE '%instagram%'")
    ...
    session_source = "cookie_db" if has_sessionid else ("storage_state" if storage_state_sessionid else "none")
    ...
    return {
        ...
        "message": "Logged in and ready" if effective_sessionid else f"Profile exists with {len(ig_cookies)} cookies but NO sessionid (not logged in)"
    }
