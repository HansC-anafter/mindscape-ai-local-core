...
async def _analyze_embedding_migration_needs(
    ...
    try:
        ...
        try:
            ...
            from ...services.embedding_migration_store import EmbeddingMigrationStore
    ...
    try:
        ...
        if not model_name:
            ...
            if embedding_setting and str(embedding_setting.value) == model_name:
                ...
                provider = "openai"
            elif "textembedding" in model_name.lower() or "embedding" in model_name.lower():
        ...
        if provider == "vertex-ai":
            # Get Vertex AI credentials from system settings (same as chat.py)
            service_account_setting = settings_store.get_setting("vertex_ai_service_account_json")
            project_id_setting = settings_store.get_setting("vertex_ai_project_id")
            location_setting = settings_store.get_setting("vertex_ai_location")
            ...
            if not vertex_service_account_json or not vertex_project_id:
                raise HTTPException(status_code=400, detail="Vertex AI credentials not configured (service_account and project_id required)")

            # Test connection with Vertex AI
            try:
                from google.cloud import aiplatform
                from google.oauth2 import service_account
                from vertexai.language_models import TextEmbeddingModel
                ...
                # Initialize credentials (same logic as VertexAIProvider)
                credentials = None
                ...
                if credentials:
                    aiplatform.init(project=vertex_project_id, location=vertex_location, credentials=credentials)
                ...
                success = bool(embeddings and len(embeddings) > 0 and embeddings[0].values)
                dimensions = len(embeddings[0].values) if success else None
                message = f"Connection successful (dimensions: {dimensions})" if success else "Connection failed"
