...
class ModelConfig(BaseModel):
    ...
    is_latest: bool = Field(
        default=False, description="Whether this is the latest model"
    )
    is_recommended: bool = Field(
        default=False, description="Whether this model is recommended"
    )
    is_deprecated: bool = Field(
        default=False, description="Whether this model is deprecated"
    )
    deprecation_date: Optional[str] = Field(
        None, description="Deprecation date (YYYY-MM-DD)"
    )
    dimensions: Optional[int] = Field(
        None, description="Dimensions (for embedding models)"
    )
    context_window: Optional[int] = Field(
        None, description="Context window size (for chat models)"
    )
...
class ModelProviderConfig(BaseModel):
    ...
    api_key_setting_key: str = Field(
        ..., description="Key in system_settings for API key (e.g., 'openai_api_key')"
    )
    base_url: Optional[str] = Field(
        None, description="Base URL for API (optional, for custom endpoints)"
    )
...
class ModelConfigCard(BaseModel):
    ...
    project_id: Optional[str] = Field(
        None, description="GCP Project ID (for Vertex AI)"
    )
    location: Optional[str] = Field(
        None, description="GCP Location/Region (for Vertex AI, e.g., us-central1)"
    )
    quota_info: Optional[Dict[str, Any]] = Field(
        None, description="Quota information (if available)"
    )
