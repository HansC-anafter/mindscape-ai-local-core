...
class ThreadOverview(BaseModel):
    ...
    summary: Optional[str] = Field(
        None, description="Auto-generated or user-edited summary"
    )
    project_id: Optional[str] = Field(None, description="Associated project ID")
    labels: List[str] = Field(default_factory=list, description="Thread labels")
    pinned_scope: Optional[dict] = Field(
        None, description="Pinned scope/connector configuration"
    )
...
class ThreadDeliverable(BaseModel):
    ...
    status: Literal["draft", "final", "archived"] = Field(
...
class ThreadRun(BaseModel):
    ...
    deliverable_ids: List[str] = Field(
        default_factory=list, description="Associated artifact IDs"
    )
    result_summary: Optional[str] = Field(
        None, description="Human-readable result summary"
    )
    storage_ref: Optional[str] = Field(
        None, description="Path to full result artifacts on disk"
    )
...
class ThreadSource(BaseModel):
    ...
    type: Literal[
        "wordpress_site", "obsidian_vault", "notion_db", "google_drive", "local_folder"
    ] = Field(..., description="Source type")
    identifier: str = Field(
        ..., description="Source identifier (site_id, vault_name, etc.)"
    )
...
class ThreadBundle(BaseModel):
    ...
    runs: List[ThreadRun] = Field(default_factory=list, description="Execution runs")
