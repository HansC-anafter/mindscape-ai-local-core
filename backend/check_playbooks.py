...
async def check_playbooks():
    ...
    if playbooks:
        ...
        if hasattr(first, "playbook"):
            print(
                f"First playbook code: {first.playbook.metadata.playbook_code if hasattr(first.playbook, 'metadata') else 'N/A'}"
            )
        elif hasattr(first, "metadata"):
            print(
                f"First playbook code: {first.metadata.playbook_code if hasattr(first.metadata, 'playbook_code') else 'N/A'}"
            )

    # PlaybookMetadata has playbook_code directly
    ig_playbooks = [
        p for p in playbooks if hasattr(p, "playbook_code") and "ig_" in p.playbook_code
    ]
    ...
    if effective:
        print(f"  First 5 with descriptions:")
        for p in effective[:5]:
            code = p.get("playbook_code")
            desc = p.get("description", "NO DESCRIPTION")
            # Truncate desc for readability
            desc_preview = (desc[:60] + "...") if len(desc) > 60 else desc
            print(f"    - {code}: {desc_preview}")

    # Specific check for ig_analyze_following
    ig_target = [
        p for p in effective if p.get("playbook_code") == "ig_analyze_following"
    ]
    if ig_target:
        p = ig_target[0]
        print(f"\nTarget Check: {p.get('playbook_code')}")
        print(f"  Name: {p.get('name')}")
        print(f"  Description: {p.get('description')}")
    else:
        print("\nTarget ig_analyze_following not found in effective playbooks!")

    # Check with None workspace
    effective_none = await scope_resolver.resolve_effective_playbooks(
        tenant_id=None, workspace_id=None, user_id="test-user-id", project_id=None
    )
