...
async def stream_openai_response(
    ...
    if not use_provider_stream:
        ...
        logger.warning(
            "Provider does not support chat_completion_stream, using direct client"
        )
        import openai

        openai_key_for_streaming = (
            provider.api_key if hasattr(provider, "api_key") else openai_key
        )
        ...
        async for chunk in stream:
            if chunk.choices and len(chunk.choices) > 0:
                ...
                if hasattr(delta, "content") and delta.content:
                    ...
                    yield f"data: {json.dumps({'type': 'chunk', 'content': delta.content, 'model_name': model_name, 'is_fallback': is_fallback})}\n\n"
...
async def stream_vertexai_response(
    ...
    if not model_name:
        ...
        logger.warning(
            f"[stream_vertexai_response] model_name was None, using fallback: {model_name}"
        )
...
def create_assistant_event(
    ...
    if thread_id:
        try:
            ...
            # logger is already available globally
...
async def handle_execution_mode_playbook_trigger(
    ...
    if execution_playbook_result:
        logger.info(
            f"[ExecutionMode] Direct playbook execution succeeded, sending result via SSE"
        )
        ...
        if execution_mode == "execution":
            ...
            if playbook_trigger_result:
                logger.info(
                    f"[PlaybookTrigger] Trigger result: {playbook_trigger_result}"
                )
...
async def stream_llm_response(
    ...
    if provider_type == "OpenAIProvider":
        ...
        logger.warning(
            f"Provider {provider_type} streaming not yet implemented for SSE"
        )
