...
async def generate_and_execute_plan(
    ...
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(
                f"\n==== GENERATE_AND_EXECUTE_PLAN ENTRY {datetime.utcnow()} ====\n"
            )
            f.write(f"Workspace: {workspace_id}\n")
            f.write(f"Execution Mode: {execution_mode}\n")
            f.write(f"Model: {model_name}\n")
            f.write(
                f"available_playbooks Count: {len(available_playbooks) if available_playbooks else 0}\n"
            )
            f.write(f"Message ID: {message_id}\n")
            f.write(f"Project ID: {project_id}\n")
            f.write("==========================================\n")
    except Exception as log_err:
        logger.warning(f"[ENTRY_LOG_ERROR] {log_err}")
    ...
    try:
        ...
        try:
            from backend.app.services.playbook.playbook_scope_resolver import (
                PlaybookScopeResolver,
            )
            ...
            if project_id:
                ...
                if project:
                    project_profile = (
                        project.metadata.get("capability_profile")
            ...
            logger.info(
                f"[ExecutionPlan] Resolved {len(effective_playbooks)} effective playbooks for streaming path"
            )
        except Exception as e:
            logger.warning(
                f"[ExecutionPlan] Failed to resolve effective playbooks: {e}, falling back to available_playbooks"
            )
        ...
        try:
            workspace = None
            try:
                workspace = await orchestrator.store.get_workspace(workspace_id)
            except Exception:
                pass
        ...
        if not execution_plan:
            logger.warning(
                f"[ExecutionPlan] generate_execution_plan returned None (plan generation failed or skipped)"
            )
            print(
                f"[ExecutionPlan] generate_execution_plan returned None (plan generation failed or skipped)",
                file=sys.stderr,
        ...
        await record_execution_plan_event(
            plan=execution_plan, profile_id=profile_id, project_id=project_id
        )
        ...
        if effective_playbooks is not None:
            if (
                not hasattr(execution_plan, "metadata")
                or execution_plan.metadata is None
            ):
                execution_plan.metadata = {}
            execution_plan.metadata["effective_playbooks"] = effective_playbooks
            execution_plan.metadata["effective_playbooks_count"] = len(
                effective_playbooks
            )
            logger.info(
                f"[ExecutionPlan] Stored {len(effective_playbooks)} effective playbooks in plan metadata"
            )
...
async def execute_plan_and_send_events(
    ...
    try:
        run_id = (
            execution_plan.id
            if hasattr(execution_plan, "id") and execution_plan.id
            else message_id
        )
        ...
        logger.info(
            f"[PipelineStage] Sent execution_start stage event, run_id={run_id}"
        )
        ...
        if execution_plan.tasks:
            agent_members = [
                ...
                if hasattr(task, "pack_id") and task.pack_id
            ]
            ...
            if len(agent_members) > 0:
                try:
                    ...
                    first_agent_name = (
                        first_agent_info.get("name_zh")
                        or first_agent_info.get("name")
                        or agent_members[0]
                        if first_agent_info
                        else agent_members[0]
                    )
                except Exception:
                ...
                if len(agent_members) > 1:
                    try:
                        ...
                        for agent_id in agent_members[1:]:
                            ...
                            if agent_info:
                                other_agent_names.append(
                                    agent_info.get("name_zh")
            ...
            logger.info(
                f"[PipelineStage] Sent task_assignment stage event, run_id={run_id}, agents={agent_members}"
            )
        ...
        def task_event_callback(event_type: str, task_data: Dict[str, Any]):
            ...
            task_updates.append({"event_type": event_type, "task_data": task_data})
        ...
        if execution_results.get("executed_tasks") or execution_results.get(
            "suggestion_cards"
        ):
            logger.info(
        ...
        yield f"data: {json.dumps(pipeline_stage_event)}\n\n"
        logger.info(
            f"[PipelineStage] Sent execution_error stage event, run_id={run_id}"
        )
