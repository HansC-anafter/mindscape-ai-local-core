...
def upgrade():
    ...
    if not inspector.has_table("meta_meeting_sessions"):
        op.create_table(
...
def downgrade():
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    if inspector.has_table("meta_meeting_sessions"):
        op.drop_index("ix_meta_meeting_sessions_created", table_name="meta_meeting_sessions", if_exists=True)
        op.drop_index("ix_meta_meeting_sessions_owner", table_name="meta_meeting_sessions", if_exists=True)
