...
down_revision: Union[str, None] = '006_add_workspace_blueprint_fields'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("category IN ('direction', 'action')", name='ck_graph_nodes_category'),
        sa.CheckConstraint("node_type IN ('value', 'worldview', 'aesthetic', 'knowledge', 'strategy', 'role', 'rhythm')", name='ck_graph_nodes_node_type'),
        sa.CheckConstraint("confidence >= 0 AND confidence <= 1", name='ck_graph_nodes_confidence'),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint("relation_type IN ('supports', 'conflicts', 'depends_on', 'related_to', 'derived_from', 'applied_to')", name='ck_graph_edges_relation_type'),
        sa.CheckConstraint("weight >= 0", name='ck_graph_edges_weight'),
        sa.UniqueConstraint('profile_id', 'source_node_id', 'target_node_id', 'relation_type', name='uq_graph_edges_no_duplicate'),
    )
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.String(30), nullable=False),
        sa.ForeignKeyConstraint(['graph_node_id'], ['graph_nodes.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('graph_node_id', 'playbook_code', name='uq_graph_node_playbook'),
    )
