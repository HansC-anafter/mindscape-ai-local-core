...
- 文件路徑: `insights_report.md`（相對路徑，相對於 sandbox 根目錄）
- 內容: 完整的洞察報告，包含：
  - 執行摘要
  - 數據綜合概述
  - 關鍵洞察和發現
...
#### 步驟 6.2: 保存行動項目
**必須**使用 `sandbox.write_file` 工具保存行動項目（首選）或 `filesystem_write_file`（需要人工確認）：

- 文件路徑: `action_items.md`（相對路徑，相對於 sandbox 根目錄）
- 內容: 優先級行動項目和實施指南
- 格式: Markdown 格式
